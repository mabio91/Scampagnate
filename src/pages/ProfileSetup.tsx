...
const ProfileSetup = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!trekkingExp || !activityFreq) {
      toast({ title: t("answerAllQuestions"), variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      await refreshProfile();
      toast({ title: t("profileSetupComplete"), description: t("welcomeToScampagnate") });
      navigate("/", { replace: true });
    } catch (error: any) {
      toast({ title: t("error"), description: error.message, variant: "destructive" });
    } finally {
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md space-y-8 bg-card p-6 rounded-2xl shadow-sm border border-border">
        ...
        <div className="text-center space-y-2">
          <h1 className="font-display text-2xl font-bold text-foreground">{t("completeYourProfile")}</h1>
          <p className="text-sm font-body text-muted-foreground">{t("fewMoreDetails")}</p>
        </div>
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="text-xs font-body text-muted-foreground">{t("uploadProfilePhoto")}</p>
          </div>

          <div className="space-y-4">
            <div>
              <Label htmlFor="phone" className="font-body text-sm font-semibold">{t("phoneNumber")}</Label>
              ...
              <div className="flex items-start gap-1.5 mt-2 text-xs text-muted-foreground bg-muted/50 p-2 rounded-lg">
                ...
                <p>{t("phoneDisclaimer")}</p>
              </div>
            </div>

            <div className="pt-4 border-t border-border">
              <h3 className="font-display font-bold text-base mb-2">{t("experienceAndActivity")}</h3>
              <p className="text-xs font-body text-muted-foreground mb-4">{t("experienceExplanation")}</p>

              <div className="space-y-4">
                <div className="space-y-2">
                  <Label className="font-body text-sm font-semibold">{t("trekkingQuestion")}</Label>
                ...
                <div className="space-y-2">
                  <Label className="font-body text-sm font-semibold">{t("activityQuestion")}</Label>
                  <div className="grid grid-cols-1 gap-2 mt-1.5">
                    {[
                      { val: ">2/week", label: t("moreThan2Week") },
                      { val: "1-2/week", label: t("oneToTwoWeek") },
                      { val: "rarely", label: t("rarely") }
                    ].map(opt => (
                      <button
          ...
          <Button type="submit" disabled={saving} className="w-full h-12 font-body font-semibold">
            {saving ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> {t("saving")}</> : t("completeSetup")}
          </Button>
