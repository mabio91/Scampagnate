    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-6">
        <div className="text-center">
          ...
          <p className="text-muted-foreground font-body">{t("invalidLink")}</p>
          <Button onClick={() => navigate("/auth")} className="mt-4">{t("backToLogin")}</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-6">
      <div className="w-full max-w-sm">
        <div className="text-center mb-6">
          <img src={logo} alt="Scampagnate" className="h-16 w-16 rounded-full mx-auto mb-3" />
          <h1 className="font-display text-2xl font-bold text-foreground">{t("newPassword")}</h1>
        </div>
        <form onSubmit={handleReset} className="space-y-4">
          <div>
            <Label className="font-body text-sm">{t("newPasswordLabel")}</Label>
            ...
            {password && (
              <div className="mt-2 space-y-2">
                ...
                <ul className="space-y-0.5">
                  ...
                  ] as [keyof typeof strength.checks, string][]).map(([key, text]) => (
          ...
          <div>
            <Label className="font-body text-sm">{t("confirmPasswordLabel")}</Label>
            <div className="relative mt-1">
          ...
          <Button type="submit" disabled={loading} className="w-full bg-primary text-primary-foreground font-body font-semibold">
            {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />{t("updating")}</> : t("updatePassword")}
          </Button>
        </form>
      </div>
    </div>
