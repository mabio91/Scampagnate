      ...
      when
        ...
        or exists (
          ...
          where es.profile_id = p.id
            and coalesce(es.is_public, true) = true
            and coalesce(e.visibility, 'public') = 'public'
        )
      then nullif(p.instagram_handle, '')
      else null
...
comment on function public.get_public_profile(uuid) is
  'Public profile summary. Organizer/staff Instagram is public; organizer contact fields are only returned for authenticated requests and contactable organizer profiles.';

comment on function public.get_public_profiles(uuid[]) is
  'Public profile summaries. Organizer/staff Instagram is public; organizer contact fields are only returned for authenticated requests and contactable organizer profiles.';
