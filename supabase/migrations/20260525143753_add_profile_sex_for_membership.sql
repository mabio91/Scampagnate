...
COMMENT ON COLUMN public.profiles.sex IS 'Sesso anagrafico M/F richiesto per tessere e assicurazione.';
COMMENT ON COLUMN public.prepaid_memberships.sex IS 'Sesso anagrafico M/F importato per tessere e assicurazione.';
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'profiles_sex_m_f_check'
      AND conrelid = 'public.profiles'::regclass
  ) THEN
    ALTER TABLE public.profiles
      ADD CONSTRAINT profiles_sex_m_f_check CHECK (sex IS NULL OR sex IN ('M', 'F'));
  END IF;
END;
$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'prepaid_memberships_sex_m_f_check'
      AND conrelid = 'public.prepaid_memberships'::regclass
  ) THEN
    ALTER TABLE public.prepaid_memberships
      ADD CONSTRAINT prepaid_memberships_sex_m_f_check CHECK (sex IS NULL OR sex IN ('M', 'F'));
  END IF;
END;
$$;
...
BEGIN
  ...
  NEW.sex := upper(NULLIF(trim(COALESCE(NEW.sex, '')), ''));
...
BEGIN
  ...
  SET
    ...
    sex = COALESCE(NULLIF(v_prepaid.sex, ''), public.profiles.sex),
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      v_sex := upper(NULLIF(trim(COALESCE(v_row ->> 'sex', '')), ''));
      ...
      IF v_sex IS NULL THEN
        v_missing_fields := array_append(v_missing_fields, 'sex');
      END IF;
