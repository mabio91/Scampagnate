...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: event, error: eventError } = await supabaseAdmin.from("events").select("cancellation_policy, date, time, title").eq("id", eventId).single();
    ...
    if (isWaitlist || ![
      "paid",
      "deposit_paid"
    ].includes(registration.payment_status || "") || !registration.stripe_payment_intent_id) {
      await supabaseAdmin.from("notifications").insert({
        user_id: user.id,
        type: "cancellation",
        title: "Iscrizione annullata",
        message: isWaitlist ? "Sei stato rimosso dalla lista d'attesa." : "La tua iscrizione è stata annullata con successo.",
        event_id: eventId
      });
      return new Response(JSON.stringify({
        refunded: false,
    ...
    try {
      const { data: freshReg } = await supabaseAdmin.from("event_registrations").select("payment_status, refund_status").eq("id", registration.id).single();
